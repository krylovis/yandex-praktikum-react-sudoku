    ...
    <ErrorBoundary>
      <main className={style.leaderPage}>
        <div className={style.leaderPage__wrap}>
          <button
            className={style.leaderPage__backWrap}
            type="button"
            onClick={handleBackBtnClick}
          >
            <img
              className={style.leaderPage__backIcon}
              src={leftBlueArrow}
              alt="Стрелка назад"
            />
          ...
          <div className={style.leaderPage__table}>
            <ul className={style.leaderPage__raw}>
              {headNames.map((headName, ind) => (
                <li
                  className={`${style.leaderPage__headText} ${
                    style.leaderPage__text
                  } ${headName === 'Имя' ? style.leaderPage__name : ''}`}
                  key={ind}
                >
                  {headName}
                </li>
            ...
            <ul className={style.leaderPage__tableContent}>
              {currentLeaders.map((leader, index) => (
                <li className={style.leaderPage__raw} key={index}>
                  <p
                    className={`${style.leaderPage__rawText} ${style.leaderPage__text}`}
                  >
                    {leader.place}
                  </p>
                  <div className={style.leaderPage__avatarWrap}>
                    <img
                      className={style.leaderPage__avatar}
                      src={avatar}
                      alt="Аватар"
                    />
                  </div>
                  <p
                    className={`${style.leaderPage__rawText} ${style.leaderPage__text} ${style.leaderPage__name}`}
                  >
                    {leader.name}
                  </p>
                  <p
                    className={`${style.leaderPage__rawText} ${style.leaderPage__text} ${style.leaderPage__lastChild}`}
                  >
                    {leader.time}
                  </p>
                </li>
